...
function getRequestToken(consumer_key, token_ready_url, request_token_url, consumer_secret)
    ...
    --//TODO: let's try breaking this in two
    --          we'll let the Request Access button get the token
    --          then have a second button to get to the webPopup
...
function rawPostRequest(url, rawdata)
        ...
        --oAuthError in API Request mis-signed: Invalid or Missing Signature 
        --could be lack of unicode normalization (NFC) or lack of UTF-8 encoding
        ...
        local function networkListener( event )
            if ( event.isError ) then
                ...
                _G.response = event.response
                --while (event.response == nil) do
                --    print("event.response is nil")
                --end
            end
        end
        ...
        --return table.concat(response,"")
        --//TODO: might need to remove table.concat
        --return table.concat(response,"")
        return response
end
...
--   need to UTF-8 encode strings passed to DropBox API
--   might need to normalize to Unicode Normalization Form C (NFC) before UTF-8 encoding
